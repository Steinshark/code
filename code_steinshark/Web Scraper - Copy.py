...
class Chickles:
        ...
        for data in self.HOA_IDs[1:]:
            ...
            try:
                ...
                for line in finalcut:
                    ...
                    self.output_writer.yview(tkinter.END)
                    for item in self.parseLine(line):
    ...
    ########################################################################################################
        ...
        ParsedData.append(Utils.scrub_name(words[3].split("<")[0][1:].title()))
        #must be encoded in quotes because contains ','
        addressFull = words[4].split("<")[0][1:]
        final_address_list = Utils.scrub_address(addressFull)
        ##          address     ##
        ParsedData.append(final_address_list[0])
        ##          City        ##
        ParsedData.append(final_address_list[1])
        ##          State       ##
        ParsedData.append(final_address_list[2])
        ##          Zip         ##
        ParsedData.append(final_address_list[3])
        ##          title       ##
        ParsedData.append(Utils.scrub_title(words[1][1:].split("<")[0].title()))
        return ParsedData
    ...
    def execute(self):
        ...
        if self.INPUT_FILE_NAME == '':
            self.output_writer.insert(tkinter.END, "No file specified\n")
            self.output_writer.yview(tkinter.END)
            return

        if self.OUTPUT_FILE_NAME == '':
            self.output_writer.insert(tkinter.END, "Save file as what?\n")
            self.output_writer.yview(tkinter.END)
            return
        try:
            self.OUTPUT_FULL_PATH = self.OUTPUT_FILE_PATH + self.OUTPUT_FILE_NAME
            self.output_writer.insert(tkinter.END, "Writing " + str(self.OUTPUT_FULL_PATH) + "\n")
            self.output_writer.yview(tkinter.END)
            ...
            self.output_writer.insert(tkinter.END, "Filepath \'" + str(self.INPUT_FILE_PATH) + str(self.INPUT_FILE_NAME) + "\' is not valid")
            self.output_writer.yview(tkinter.END)
            pass
        return
    ...
    def cancel_execution(self):
        if self.build_job == None:
            self.output_writer.insert(tkinter.END, "No Thread Running\n")
            self.output_writer.yview(tkinter.END)
            return
        if self.build_job.is_alive():
            self.thread_running = False
        else:
            self.output_writer.insert(tkinter.END, "There was an error killing thread\n")
            self.output_writer.yview(tkinter.END)


    def get_file(self):
            file = askopenfile(filetypes=(("csv files","*.csv"),("All files","*.*")))
            self.output_writer.insert(tkinter.END, '\'' + str(file.name).replace('/', '\\') + "\' selected\n")
            self.output_writer.yview(tkinter.END)
            print(file)
    ...
    def get_directory(self):
            ...
            self.output_writer.insert(tkinter.END, 'Saving to \'' + str(self.OUTPUT_FILE_PATH) + "\'\n")
            self.output_writer.yview(tkinter.END)
    ...
    def make_gui(self):
        ...
        self.output_writer.insert(tkinter.END, '\'' + str(self.INPUT_FILE_PATH) + "\' being used as default save path\n")
        self.output_writer.yview(tkinter.END)
