...
class Graph:
    ...
    def big_bfs(self,node_a):
        open = [node_a]
        full_list = self.assetNodes + self.targetNodes
        paths = {x : (None,None) for x in full_list if not x == node_a}
        discovered = {node_a:True}
        parents = {}
        while (None,None) in paths.values():
            node = open.pop(0)
            if node in full_list and not node == node_a:
                path = []
                p = parents[node]
                while not p[0] == node_a:
                    path.insert(0,p[1])
    ...
    def min_loss(self):
        ...
        self.min_loss = 99999
        self.min_loss_rec(copy.deepcopy(self.graph_verts),copy.deepcopy(self.targetNodes),0)

    def min_loss_rec(verts,remaining,loss):
        ...
        if len(remaining) == 0:
            ...
            for n2 in remaining:
                for asset in iter_verts.keys():
                    ...
                    new_list = copy.deep_copy(iter_verts)
                    new_list.append(node)

                    loss += len(iter_rem)*cost + sum(deaths)
            ...
            for divy in itertools.permutations(remaining,len(verts)):
                iter_verts = copy.deepcopy(verts)
                i = 0
                for node in iter_verts:
                    n1 = self.iter_verts[node][-1]
                    n2 = copy.copy(divy[i])
                    iter_verts[node].append(n2)
                    cost = self.get_edge(n1,n2).len
