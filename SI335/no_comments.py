file = open("rooms.cpp",'r')
file_write = open("newRooms.cpp",'w')
for line in file.readlines():
    file_write.write(str(line.split(r"//")[0]) + "\n")
file_write.close()
file.close()
...
coMatcherList reOrderMatchers(roomList midnList, coMatcherList matchers){     //
  ...
  int coBeingPlaced = 1;                                                      //
  int curIndex = 0;                                                           //
  for(coBeingPlaced; coBeingPlaced <= totalCompanies; coBeingPlaced++){       // Will place one matcher per iteration
    int bestMatchCo = 0;                                                      //
    int maxMatches = -1;                                                      //
    for(int coNumber = 1; coNumber <= totalCompanies; coNumber++){            // will find which copmany this space best matches
